    ...
    <div class="">
        <b-row>
            <b-col md="11" class="my-1 ml-1">
                <b-form-group horizontal label="Recherche par adresse email de l'utilisateur " class="mb-0">
                    <b-input-group>
                         <b-form-input v-model="useremail" placeholder="Recherche..." />
                        <b-input-group-append class="apend-btn ml-2">
                            <b-btn class="submit-btn" @click="search_devis()">Recherche</b-btn>
        ...
        <b-row v-if="listdevis" class="mt-2 border ml-1 mr-2">
            <h3 class="ml-3" style="font-weight:bold"> 
                <span style="color:red">{{totalRows}}</span> 
                devis trouvé(s) 
            </h3>
        </b-row>
...
export default {
    ...
    methods: {
        ...
        search_devis()
        {
            ...
            {
                Api().post('/devis/search/useremail', {email:this.useremail})
                .then((result) => {
                    if(result.data.code==1)
                    {
                        this.listdevis = result.data.devis
                        this.totalRows = result.data.devis.length
                    }else{
